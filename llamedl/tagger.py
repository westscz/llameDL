...
class Tagger:
    ...
    def get_tags(self, artist_name):
        tag_list = self.__get_tags_from_last_fm(artist_name)
        tag_list += self.__get_tags_from_musicbrainzgs(artist_name)
    ...
    def __get_tags_from_musicbrainzgs(self, artist_name):
    ...
    def __get_tags_from_last_fm(self, artist_name):
        artist_name = artist_name.replace(" ", "%20")
        api_key = "a7b8b15b54a347be1736dd0f77c7d048"
        x = requests.get(
            "http://ws.audioscrobbler.com/2.0/?"
            "method=artist.getInfo&artist={artist}&user=RJ&api_key={api_key}&format=json".format(
                artist=artist_name, api_key=api_key))
        if x.json().get('error'):
            return []
        tags = x.json().get('artist').get('tags').get('tag', [])
...
if __name__ == '__main__':
    x = Tagger()
    band_list = ['Blade Rain', 'Red Hot Chili Peppers', 'Skrillex', 'asdsgfdasdfg', 'Desiigner', "Taco Hemingway",
